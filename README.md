...
- _QuestionService_ is responsible for managing "Question" entity in DB - creating, finding one, updating and removing questions. It uses injected _AnswerService_ to manage answers (because they depend on questions).
- _QuizService_ is responsible for managing "Quiz" entity in DB - creating, finding one, finding all, updating and removing quizes. Like _QuestionService_ it sometimes uses injected _QuestionService_ to manage questions.
- _AnswerSubmitterService_ allows to submit answers and to get a result. It checks if given answer for specific question is correct.
...
- _QuizResolver_ allows to create quiz (with or without questions), find one/all, update or remove quiz.
- _AnswerSubmitterResolver_ allows to submit answers

**Tests**  
To run use `npm run test` or `npm test -- --testPathPattern=src/question/question.service.spec.ts` for specific file.  
Tested are: QuestionService, QuizService and AnswerSubmitterService. Resolvers had to be tested "manually" (in GraphQL Playground).  
